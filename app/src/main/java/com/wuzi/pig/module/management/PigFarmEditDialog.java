...
public class PigFarmEditDialog extends BaseDialogFragment<PigFarmPresenter> implements PigFarmContract.IView {
    ...
    protected int getLayoutID() {
        return R.layout.dialog_pigfarm_edit;
    }
    ...
    protected void initView(View view, Bundle savedInstanceState) {
        ...
        if (mPigFarmEntity != null) {
            mNameValueView.setText(StringUtils.nullToString(mPigFarmEntity.getPigfarmName()));
        }
    }
        ...
        switch (v.getId()) {
            case R.id.ok:{
                ...
                if (name.length() <= PigFarmConstant.PIG_FARM_NAME_MAX_LENGTH) {
                    mPresenter.updatePigFarm(mPigFarmEntity.getPigfarmId(), name);
                }
                break;
            }
            case R.id.cancel:{
    ...
    public void performSuccess(int fromTag) {
        ...
        if (mSubmitListener != null) {
            mSubmitListener.action(mNameValueView.getText().toString());
        }
    }
